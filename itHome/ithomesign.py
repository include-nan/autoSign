...
class IIhomeSign():
    ...
    def sign(self):
        endt = des_encrypt(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        headers = {
            'Referer': 'https://my.ruanmei.com/app/user/signin.html?hidemenu=1&appver=2'
        }
        url = self.urlhead + "api/usersign/sign?userHash=" + self.userhash + "&type=0" + self.getSign()
        result = requests.get(url, headers=headers)
        return result.content

    def signlapin(self):
        endt = des_encrypt(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        url = self.urlhead + "/api/usersign/sign?userHash=" + self.userhash + "&type=1&platform=lapinapp_android&channel=official" + self.getSign()
        headers = {
            'Referer': 'https://my.ruanmei.com/app/user/signin.html?hidemenu=1&appver=2'
        }
        result = requests.get(url, headers=headers)
    ...
    def signyunrili(self):
        endt = des_encrypt(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        url = self.urlhead + "api/usersign/yunrilisign?userHash=" + self.userhash + "&coinHistoryType=160&appver=3.03&platform=windows" + self.getSign()
        headers = {
            'Referer': 'https://my.ruanmei.com/app/user/signin.html?hidemenu=1&appver=2'
        }
        result = requests.get(url, headers=headers)
    ...
    def signyunriliAndroid(self):
        endt = des_encrypt(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        url = self.urlhead + "api/usersign/yunrilisign?userHash=" + self.userhash + "&coinHistoryType=159" + self.getSign()
        headers = {
...
if __name__ == "__main__":
    ...
    if msg.find("失败".encode()) > -1:
        notify.send(title=u"#it之家签到", content=msg)
    else:
        notify.send(title=u"#it之家签到", content=msg)
