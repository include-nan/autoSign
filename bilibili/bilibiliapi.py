                    ...
                    if response and response.get("code") is not None:
                        if response['code'] == -105:
                            ...
                            if captcha:
                                ...
                                self._log(
                                    f"登录验证码识别服务暂时不可用, {'尝试更换代理' if self.proxy else '10秒后重试'}")
                ...
                try:
                    ...
                    if result['bottom-buy-button']:
                        if "bottom-buy-disable" not in result['bottom-buy-button'].get_attribute("class"):
                            ...
                            if in_stock:
                                self._log(
                                    f"(线程{thread_id})商品{item_id}暂无法购买, 原因为{result['bottom-buy-button'].text}")
                                in_stock = False
                            driver.refresh()
                            timestamp = time.time()
                    if result['pay-btn']:
                    ...
                    if result['expire-time-format']:
                        self._log(
                            f"(线程{thread_id})商品{item_id}订单提交成功, 请在{result['expire-time-format'].text}内完成支付")
                        driver.quit()
                        return True
            ...
            while not flag:
                ...
                if response and response.get("code") is not None:
                    if response['code'] == 83094004:
                        ...
                        self._log(
                            f"(线程{thread_id})会员购优惠卷\"{coupon_info['name']}\"(ID={coupon_id})领取失败, 优惠券领取数量已达到上限")
                    elif response['code'] == 83110015:
                        self._log(
                            f"(线程{thread_id})会员购优惠卷\"{coupon_info['name']}\"(ID={coupon_id})领取失败, 优惠券库存不足")
                    else:
                        continue
                else:
                    self._log(
                        f"(线程{thread_id})会员购优惠卷\"{coupon_info['name']}\"(ID={coupon_id})领取失败, 当前IP请求过于频繁")
                flag = True
    ...
    def mall_order_list(self, status=0, type=[2]):
        ...
        def get_order_detail(order_id):
            ...
            if response and response.get("code") == 0 and response['data']['vo']:
                ...
                self._log(
                    f"会员购订单{order_id}详情获取成功, 包含\"{data['skuList'][0]['itemsName']}\"等{len(data['skuList'])}件商品")
                return data
            else:
            ...
            if response and response.get("code") == 0:
                ...
                for coupon in coupon_list:
                    self._log(f"会员购优惠券: {coupon['name']}" + (
                        f", 失效时间为{coupon['expire']}" if coupon['expire'] else f", 使用时间为{coupon['use']}" if
                        coupon['use'] else f", 使用有效期为{coupon['start']}至{coupon['end']}" if coupon['start'] and
                                                                                                  coupon[
                                                                                                      'end'] else ""))
                break
            else:
        ...
        while True:
            ...
            if response and response.get("code") == 0:
                ...
                if response['data']['pageInfo']['hasNextPage']:
                    ...
                    self._log(
                        f"会员购奖品列表获取成功, 总计{len(prize_list)}个奖品, {response['data']['waitDeliveryNum']}个奖品待发货")
...
def wrapper(arg):
    ...
    if instance.login(force_refresh_token=config['user']['force_refresh_token'], **account):
        ...
        if config['set_privacy']['enable']:
            ...
            config['set_privacy']['show_tag'], config['set_privacy']['show_reward'], config['set_privacy']['show_info'],
