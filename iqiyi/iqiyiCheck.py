...
if __name__ == "__main__":
    if os.environ.get('IQIYI_COOKIE'):
        _check_item = os.environ.get('IQIYI_COOKIE')
        message = IQIYI(check_item=_check_item).main()
        notify.send(title=u"#爱奇艺", content=message)
    else:
        notify.send(title=u"#爱奇艺", content="未找到cookie")
